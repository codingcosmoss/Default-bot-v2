...
<!--                            <i @click = "this.$router.push({ path: '/service-category/show', query: { id: item.id } })" class="fa-solid setting-icon fa-eye"></i>-->
    ...
    <ModalLayout @onButton = "crudCategory()" :isModal = "isModal" @closeModal = "isModal = false" :Title = " crud == 'created' ? getName('create') :getName('update')" >
        ...
        <Select
            ...
            :isError="hasKey('warehouse_category_id')"
            :message="errorObj['warehouse_category_id']"
            @onSelect="category_id = $event"
        >
            <option>---</option>
...
import {
    ...
    warehouseShow,
    warehousesSearch,
    warehouseUpdate, warehouseCreate, warehouseDelete,warehouseCategorys, filials
} from "../../../Api.js"
...
export default {
    ...
    methods:{
        ...
        async getModel(){
            const response = await warehouseShow(this.UpdatedId);
            this.Name = response.data.name;
            this.Address = response.data.address;
            this.category_id = response.data.category_id;
            this.filial_id = response.data.filial_id;
            this.status = response.data.status == 1 ? true : false;
        },
        onUpdateModal(id){
            this.UpdatedId = id;
        ...
        async crudCategory(){
            ...
            var data = {
                ...
                'warehouse_category_id': this.category_id ,
            }
            if(this.crud == 'created'){
                response = await warehouseCreate(data);

            }else {
                response = await warehouseUpdate( this.UpdatedId, data);
            }
            if (response.status){
                this.Name = '';
                this.category_id = '';
        ...
        async getItems(){
            const response = await warehouses(this.currentPage, this.paginateCount);
        ...
        async getFilials(){
            const response = await filials(1, 1000);
            this.Filials = response.data.items;
        },
        async getCategories(){
            const response = await warehouseCategorys(1, 1000);
        ...
        async clickSearch(val, order){
            ...
            if (val != ''){
                ...
                const response = await warehousesSearch(data);
                console.log(response)
                this.items = response.data.items;
            }else {
                this.getItems()
        ...
        async onDelete(val){
            ...
            if (confirm(message)){
                const response = await warehouseDelete(val);
