...
<!--                    </div>-->
                    ...
                    <div class="flex items-center justify-center p-2.5 sm:flex xl:p-5">
                        <p class="font-medium text-meta-5">
                            <i v-if="item.status == 14 && hasPermission('Warehouse-update')" @click = "onModal(item.id)" class="fa-solid setting-icon fa-pen-to-square"
                            ></i>
                            &nbsp;
                            <i :title = "getName('AddProduct')" @click="this.$router.push({ path: '/producy-documents/add-products', query: { id: item.id } })" class="fa-solid fa-file-invoice setting-icon"
