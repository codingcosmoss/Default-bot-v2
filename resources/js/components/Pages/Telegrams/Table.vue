<template >
    ...
    <ModalLayout @onButton = "crudCategory()" :isModal = "isModal" @closeModal = "isModal = false" :Title = " crud == 'created' ? getName('create') :getName('update')" >
        ...
        <Input
            :Label = "getName('TelegramId')"
            @onInput = "telegramId = $event"
            :isError = "hasKey('telegram_id')"
            :message = "errorObj['telegram_id']"
            :Value = "telegramId"
            Pholder = "Telegram Id"
        />
...
import {
    ...
    telegrams,
    telegramShow,
    telegramUpdate, telegramCreate, telegramSearch
} from "../../../Api.js"
...
export default {
    ...
    methods:{
        ...
        async getModel(){
            const response = await telegramShow(this.UpdatedId);
            this.categoryName = response.data.name;
            this.telegramId = response.data.telegram_id;
        },
        ...
        async crudCategory(){
            ...
            if(this.crud == 'created'){
                response = await telegramCreate(data);

            }else {
                response = await telegramUpdate( this.UpdatedId, data);
            }
        ...
        async getItems(){
            const response = await telegrams(this.currentPage, this.paginateCount);
        ...
        async clickSearch(val, order){
            ...
            if (val != ''){
                ...
                const response = await telegramSearch(data);
        ...
        async onDelete(val){
            ...
            if (confirm(message)){
                const response = await telegramDelete(val);
