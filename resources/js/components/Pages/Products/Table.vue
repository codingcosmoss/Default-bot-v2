      ...
      <div class="flex flex-col">
        <div class="grid grid-cols-3 rounded-sm mb-2 sm:grid-cols-5 relative">
          ...
          <div>
            ...
            >
              <option value="name">{{ getName("name_content") }}</option>
        ...
        >
          ...
          <div class="flex items-center justify-center p-2.5 xl:p-5">
            <p class="font-medium text-black dark:text-white">{{ item.group }}</p>
          </div>
...
import ShowForm from "@/components/Pages/Products/Show/ShowForm.vue";
export default {
  components: {
      ShowForm,
      WarehouseTopbar,
  ...
  methods: {
    ...
    async getItems() {
      const response = await products(this.currentPage, this.paginateCount);
    ...
    async onDelete(val) {
      ...
      if (confirm(message)) {
        const response = await productDelete(val);
