            ...
            <div class="flex flex-col table_media_box">
                ...
                <div class="grid grid-cols-3 rounded-sm mb-2 sm:grid-cols-5 relative media_top_bar">
                    ...
                    <div class="flex flex-wrap gap-5 xl:gap-20" style="position: absolute; right: 0px; top: 0"
                         v-if="hasPermission('Diseases-create')"
                    >
                ...
                <div v-for="(item, index) in items" class=" databes_table  grid grid-cols-5 border-b border-stroke dark:border-strokedark sm:grid-cols-5 databes_table">
                    ...
                    <div class="flex items-center justify-center p-2.5 xl:p-5">
                        <p class="font-medium text-black dark:text-white">{{item.patients}}</p>
                    </div>
                    ...
                    <div class="flex items-center justify-center p-2.5 sm:flex xl:p-5">
                        <p class="font-medium text-meta-5">
                            ...
                            <i @click = "onModal(item.id)" class="fa-solid setting-icon fa-pen-to-square"
                               v-if="hasPermission('Diseases-update')"
                            ></i>
                            &nbsp;
<!--                            <i @click = "this.$router.push({ path: '/employees/show', query: { id: item.id } })" class="fa-solid setting-icon fa-eye"></i>-->
<!--                            &nbsp;-->
                            <i
                                v-if="hasPermission('Diseases-delete')"
                                @click = "onDelete(item.id)" class="fa-solid text-danger fa-trash setting-icon"></i>
                        </p>
                    </div>
...
export default {
    ...
    methods:{
        ...
        async getItems(){
            const response = await diseases(this.currentPage, this.paginateCount);
        ...
        async clickSearch(val, order){
            ...
            const response = await diseasesSearch(data);
            this.items = response.data.items;
        ...
        async onDelete(val){
            ...
            if (confirm(message)){
                const response = await diseaseDelete(val);
