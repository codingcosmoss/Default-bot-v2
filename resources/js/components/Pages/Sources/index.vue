                ...
                <div class="table-responsive">
                    <table class="table table-bordered mb-0">
                        ...
                        <tr>
                            <th>#</th>
                            <th >Nomi</th>
                            <th style="min-width: 200px">Izoh</th>
                            <th>So'zlar soni</th>
                            <th>Saqlanganlar soni</th>
                            <th>Sozlamalar</th>
                        </tr>
                        </thead>
                        <tbody>
                        ...
                        <tr v-for="(item, index) in items">
                            ...
                            <td>{{ item.text }}</td>
                            <td>{{ item.words }} ta</td>
                            <td>{{ item.save_words }} ta</td>
                            <td>

                                <button v-if="item.name != 'Manual' "  type="button" class="btn btn-primary waves-effect waves-light setting_btn" data-bs-toggle="modal" data-bs-target="#update" @click="model=item">
                                    <i class="bx bx-edit-alt"></i>
                                </button>
                                &nbsp;
                                <button v-if="item.words == 0 && item.name != 'Manual' " type="button" class="btn btn-danger waves-effect waves-light setting_btn"  @click="model=item" data-bs-toggle="modal" data-bs-target="#delete" >
...
<script>
    ...
    import {sourceDelete, sources, sourceCreate, sourceSearch, sourceShow, sourceUpdate} from "../../../Api.js";
        ...
        methods:{
            async index(){
                try {
                    ...
                    const response = await sources(this.currentPage , 10);
            ...
            async destroy(){
                try {
                    const response = await sourceDelete(this.model.id);
                    if (response.status){
            ...
            async search(val){
                try {
                    ...
                    const response = await sourceSearch(data);
                    this.items = response.data.items;
                }catch (e){
                    useConterStore().sendError(this,e);
                }
