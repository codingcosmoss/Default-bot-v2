                    ...
                    <div class="flex items-center justify-center p-2.5 sm:flex xl:p-5">
                        <p class="font-medium text-meta-5">
                            ...
                            <i @click = "onDelete(item.id)" class="fa-solid text-danger fa-trash setting-icon"
                               v-if="hasPermission('Warehouse-delete') && item.products.length == 0"
                            ></i>
                        </p>
                    </div>
...
import {
    ...
    groups,
    groupSearch,
    groupUpdate, groupCreate, groupDelete
} from "../../../Api.js"
...
export default {
    ...
    methods:{
        ...
        async getModel(){
            const response = await groupShow(this.UpdatedId);
        ...
        async crudCategory(){
            ...
            if(this.crud == 'created'){
                response = await groupCreate(data);

            }else {
                response = await groupUpdate( this.UpdatedId, data);
            }
        ...
        async getItems(){
            const response = await groups(this.currentPage, this.paginateCount);
        ...
        async clickSearch(val, order){
            ...
            if (val != ''){
                ...
                const response = await groupSearch(data);
                console.log(response)
                this.items = response.data.items;
            }else {
                this.getItems()
        ...
        async onDelete(val){
            ...
            if (confirm(message)){
                const response = await groupDelete(val);
