<template>
    <Page Title="">
        <div class="row"  >
            <BasicTable
                :Th="[ $t('Id'),$t('Image'),$t('Name'),$t('Phone'),$t('Address'),$t('Settings')]"
                :Title="$t('Suppliers')"
                Col="col-lg-12"
            >
                ...
                <template v-slot:buttons>
                    <PrimaryBtn v-if="counterStore.hasRole('Suppliers-create')" role="button" data-bs-toggle="modal" data-bs-target="#supplierCreate" >{{$t('Create')}}</PrimaryBtn>
                </template>
                ...
                <tr v-for="item in items" >
                    ...
                    <td>
                        ...
                        <PrimaryIconBtn v-if="counterStore.hasRole('Suppliers-delete')" @click="this.delete(item.id)" class="bg-danger border-danger" Icon="bx bx-trash-alt"/>
                    </td>
...
<script>
    ...
    import {suppliers, supplierCreate, supplierSearch, supplierUpdate, supplierShow, supplierDelete, supplierPaginates, supplierActives, supplierOrderBys} from "@/helpers/api.js";
    ...
    export default {
        ...
        methods:{
            ...
            async indexActives(){
                try {
                    const response = await supplierActives(this.paginateCount);
                    this.items = response.data;
                }catch(error){
                    ApiError(this, error);
                }
            ...
            async indexPaginates(page=1){
                try {
                    this.loader = true;
                    const response = await supplierPaginates(this.paginateCount, page);
            ...
            async orderBys(){
                try {
                    const response = await supplierOrderBys(this.column, this.type);
                    this.items = response.data;
                }catch(error){
                    ApiError(this, error);
                }
            ...
            async update(){
                try {
                    ...
                    const response = await supplierUpdate(data);
                    if (response.status){
            ...
            async search(text = ''){
                try {
                    ...
                    const response = await supplierSearch(text);
                    this.items = response.data;
                    this.loader = false;
                    if (!response.status){
                        Alert('error', this.$t('formError'));
            ...
            async delete(id){
                try {
                    ...
                    const response = await supplierDelete(id);
                    this.items = response.data;
                    if (response.status){
