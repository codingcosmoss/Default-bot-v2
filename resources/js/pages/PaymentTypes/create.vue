<template>
    <!--    Modal   -->
    <ModalCentered :Title="$t('Create')" ModalName="payment_typeCreate"  :isModalFooter="false">
        <div class="row"  >
            <BaseBox Col="col-xl-12" Title="">
                ...
                <div class="row">
                    ...
                    <div class="col-12 d-flex flex-column">
                        ...
                        <input type="checkbox" id="payment_type_create" switch="none"  @input="status = status == 1 ? 0 : 1" :checked="status == 1 ? true : false" >
                        <label for="payment_type_create" data-on-label="On" data-off-label="Off"></label>
                    </div>
                </div>
    ...
    methods: {
        ...
        async create() {
            try {
                ...
                const response = await payment_typeCreate(data);
                if (response.status) {
                    Alert('success', this.$t('create'));
                    this.save();
                    this.loader = false;
                    this.errors = [];
                    this.counterStore.hiddenModal('payment_typeCreate');
