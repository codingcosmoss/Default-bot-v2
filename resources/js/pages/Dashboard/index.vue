    ...
    <Page Title="">
        ...
        <div class="row " v-if="!cardLoader">
            <div class="col-md-3 cursor-pointer" @click="this.$router.push('/admin/medicines')">
                <CardBlock :Title="$t('RegisteredMedicines')" :Number="counterStore.formatNumber(registeredMedicines)"
                    Icon="bx bx-copy-alt font-size-24" />
            </div>
            <div class="col-md-3 cursor-pointer" @click="this.$router.push('/admin/expired-medicines')">
                <CardBlock :Title="$t('ExpiredMedicines')" :Number="counterStore.formatNumber(expiredMedicinesCount)"
                    Icon="bx bx-time font-size-24" />
            </div>
            <div class="col-md-3 cursor-pointer" @click="this.$router.push('/admin/import/medicines')">
                <CardBlock :Title="$t('ImportedDrugs')" :Number="counterStore.formatNumber(importedMedicines)"
                    Icon="bx bx-import font-size-24" />
            </div>
            <div class="col-md-3 cursor-pointer" @click="this.$router.push('/admin/medicines')">
                <CardBlock :Title="$t('WarehouseAvailableMedicines')" :Number="counterStore.formatNumber(realMedicines)"
        ...
        <div class="row" v-if="!cardClassicLoader">
            <CardClassicBlock Icon="bx bx-export" :Title="$t('TotalPaid')" :Currencies="medicinesSoldCustomers"
                :Share="medicinesSoldCustomersAmount" :Text="$t('SubtotalAmount') + ':'" />
            <CardClassicBlock Icon="bx bx-import" :Title="$t('ImportPaid')" :Currencies="importedSubtotalMedicines"
                :Share="importedSubtotalMedicinesAmount" :Text="$t('SubtotalAmount') + ':'" />
            <CardClassicBlock Icon="bx bx-collection" :Title="$t('TotalCosts')" :Currencies="expenses" Share="" Text="" />
            <CardClassicBlock Icon="bx bx-collection" :Title="$t('TotalNetProfit')" :Currencies="totalNetProfit" Share=""
                Text="" />
        </div>
        ...
        <div class="row">
            ...
            <BaseBox Col="col-xl-6" :Title="$t('BestSellers')">
                <div id="dashboardChart0" v-if="isChart0">
                    <apexchart type="bar" height="345" :options="chart0Data.chartOptions" :series="chart0Data.series">
                    </apexchart>
                </div>
            </BaseBox>
            ...
            <BaseBox :Title="$t('MostSoldMedicines')">
                <div id="dashboardChart2" v-if="isChart2">
                    <apexchart type="bar" height="345" :options="chart2Data.chartOptions" :series="chart2Data.series">
                    </apexchart>
                </div>
            </BaseBox>
        ...
        <ModalCentered :isSubmit="false" ModalName="dashboardAlert" :Title="$t('Info')">
            <div class="col-xl-12">
                <div class="card">
...
export default {
    ...
    methods: {
        ...
        chart1() {
            this.signsArr.forEach(e => {
                let sellings = this.sellings.filter(item => item.currency == e);
                let purchases = this.purchases.filter(item => item.currency == e);

                sellings.forEach(data => delete data.currency);
                purchases.forEach(data => delete data.currency);
                ...
                var chart = new ApexCharts(document.querySelector("#dashboardChart1" + e), options);
                chart.render();
            ...
            try {
                ...
                if (response.status) {
                    ...
                    data.sellings.forEach(e => {
                        ...
                        if (!this.signsArr.includes(e.title)) {
                            this.signsArr.push(e.title);
                        }
