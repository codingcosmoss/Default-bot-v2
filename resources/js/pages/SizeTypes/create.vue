<template>
    <!--    Modal   -->
    <ModalCentered :Title="$t('Create')" ModalName="size_typeCreate"  :isModalFooter="false">
        <div class="row"  >
    ...
    methods: {
        ...
        async create() {
            try {
                ...
                const response = await size_typeCreate(data);
                if (response.status) {
                    Alert('success', this.$t('create'));
                    this.save();
                    this.loader = false;
                    this.errors = [];
                    this.counterStore.hiddenModal('size_typeCreate');
