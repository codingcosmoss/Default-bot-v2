<template>
    <!--    Modal   -->
    <ModalCentered :Title="$t('Create')" ModalName="boxSizeCreate"  :isModalFooter="false">
        <div class="row"  >
...
import {
    ...
    userOrderBys,
    userUpdatePassword,
    size_typeCreate, box_sizeCreate
} from "../../helpers/api.js";
    ...
    methods: {
        ...
        async create() {
            try {
                ...
                const response = await box_sizeCreate(data);
                if (response.status) {
                    Alert('success', this.$t('create'));
                    this.save();
                    this.loader = false;
                    this.errors = [];
                    this.counterStore.hiddenModal('boxSizeCreate');
