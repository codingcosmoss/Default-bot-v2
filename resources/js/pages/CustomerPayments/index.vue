<template>
    <Page Title="">
        <div class="row">
        <BasicTable
                :Th="[ $t('No'),$t('Customer'),$t('Employee'),$t('Invoice'),$t('AmountPaid'),$t('PaymentType'), $t('PaidDate')]"
            ...
            >
                ...
                <tr v-for="(item,i) in items" >
                    ...
                    <td class="text-success cursor-pointer"  @click="this.$router.push({path:'/admin/invoices/show', query:{id: item.invoice_id}})" >{{ item.invoice_id }}</td>
...
<script>
    ...
    import {payment_types, payment_typeCreate, payment_typeSearchCustomers, payment_typeDelete, payment_typePaginates, payment_typeActives, payment_typeCustomers} from "@/helpers/api.js";
        ...
        methods:{
            async indexPaginates(page=1, islaoder = true){
                try {
                    ...
                    const response = await payment_typeCustomers(50, page);
                    if (response.status){
