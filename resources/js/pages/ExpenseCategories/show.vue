<template>
    <Page Title="" :Links=" [
        ...
        <div class="row"  >
            ...
            >
                ...
                <Paginate
                    ...
                    @changePage="getCategoryExpensies($event)"
                />
        ...
        <Update :Item="item" @onUpdate="getCategoryExpensies(this.current_page)" />
        <Create  @onCreate="getCategoryExpensies(this.current_page)" />
    </Page>
</template>
<script>
...
import {
    ...
    expense_categorys,
    getCategoryExpenses,
    expense_categoryExpensesSearch,
    expense_categorySearch
} from "@/helpers/api.js";
...
import Update from "../Expenses/update.vue";
import Create from "../Expenses/create.vue";
    ...
    methods:{
        ...
        async search(text = ''){
            try {
                this.loader = true;
                if (text == ''){
                    this.getCategoryExpensies();
                    return true;
                }
                this.text = text;
                const response = await expense_categoryExpensesSearch(this.$route.query.id ,text);
                this.items = response.data;
                this.loader = false;
                if (!response.status){
                    Alert('error', this.$t('formError'));
        ...
        async delete(id){
            try {
                ...
                if (response.status){
                    ...
                    this.getCategoryExpensies(this.current_page)
                }
                Alert('error', this.$t('formError'));
                return false;
            }catch(error){
        ...
        async getCategoryExpensies(page=1){
            try {
                this.loader = true;
                const response = await getCategoryExpenses(this.$route.query.id, page);
                this.loader = false;
                this.current_page = response.data.pagination.current_page;
                this.last_page = response.data.pagination.last_page;
                this.items = response.data.items;
