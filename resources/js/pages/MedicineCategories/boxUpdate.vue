<template>
    <!--    Modal   -->
    <ModalCentered :Title="$t('Update')" ModalName="drug_companyUpdate"  :isModalFooter="false">
        <div class="row"  >
            <BaseBox Col="col-xl-12" Title="">
                ...
                <div class="row">
                    ...
                    <div class="col-12 d-flex flex-column">
                        ...
                        <input type="checkbox" id="drug_company_update" switch="none" @input="status = status == 1 ? 0 : 1" :checked="status == 1 ? true : false" >
                        <label for="drug_company_update" data-on-label="On" data-off-label="Off"></label>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        async show(id) {
            try {
                const response = await drug_companyShow(id);
        ...
        async update() {
            try {
                ...
                const response = await drug_companyUpdate(this.Item.id, data);
                if (response.status) {
                    Alert('success', this.$t('create'));
                    // this.show(response.data.id);
                    this.errors = [];
                    this.loader = false;
                    this.counterStore.hiddenModal('drug_companyUpdate');
